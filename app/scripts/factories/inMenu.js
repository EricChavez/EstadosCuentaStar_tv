...
angular.module('softvApp')
	.factory('inMenu', function($localStorage) {
		var factory = {};
		var permisos = [];
		...
		factory.on = function() {
			$localStorage.currentUser.Menu.forEach(function(item) {
				...
				if (item.Title === 'Facturación') {
					item.MenuChild.forEach(function(fact) {
						var factTitle = removeDiacritics(fact.Title);
						if (fact.OptAdd) {
							permisos.push(factTitle.replace(/\s/g, '').toLowerCase() + 'Add');
						}
						if (fact.OptDelete) {
							permisos.push(factTitle.replace(/\s/g, '').toLowerCase() + 'Delete');
						}
						if (fact.OptUpdate) {
							permisos.push(factTitle.replace(/\s/g, '').toLowerCase() + 'Update');
						}
						if (fact.OptSelect) {
							permisos.push(factTitle.replace(/\s/g, '').toLowerCase() + 'Select');
						}
					});
				}
				if (item.Title === 'Procesos') {
					item.MenuChild.forEach(function(proce) {
						var ProceTitle = removeDiacritics(proce.Title);
						if (proce.OptAdd) {
							permisos.push(ProceTitle.replace(/\s/g, '').toLowerCase() + 'Add');
						}
						if (proce.OptDelete) {
							permisos.push(ProceTitle.replace(/\s/g, '').toLowerCase() + 'Delete');
						}
						if (proce.OptUpdate) {
							permisos.push(ProceTitle.replace(/\s/g, '').toLowerCase() + 'Update');
						}
						if (proce.OptSelect) {
							permisos.push(ProceTitle.replace(/\s/g, '').toLowerCase() + 'Select');
						}
					});
				}
				if (item.Title === 'Reportes') {
					item.MenuChild.forEach(function(report) {
						var reportTitle = removeDiacritics(report.Title);
						if (report.MenuChild.length === 0) {
							if (report.OptAdd) {
								permisos.push(reportTitle.replace(/\s/g, '').toLowerCase() + 'Add');
							}
							if (report.OptDelete) {
								permisos.push(reportTitle.replace(/\s/g, '').toLowerCase() + 'Delete');
							}
							if (report.OptUpdate) {
								permisos.push(reportTitle.replace(/\s/g, '').toLowerCase() + 'Update');
							}
							if (report.OptSelect) {
								permisos.push(reportTitle.replace(/\s/g, '').toLowerCase() + 'Select');
							}
						} else {
